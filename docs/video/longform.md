...
| voice | `mia` (`af_heart`) female | **candidate, not approved**; see the project docs. **There is no male explainer voice** - see the project docs |
...
sits in a margin column and its leader stops at a dot on the image border -
nothing crosses into the picture (rebuilt three times, 2026-09-10; the
rejected versions are in `beats.md`). Reach for it wherever a specific claim
currently runs under an unlabelled photo - which the section above already
names as the thing that reads as hard to follow - and keep each label to two
or three words. `x` picks the side and `y` places the dot on that edge; a
wrong region is a script note, not something the layout can save.
